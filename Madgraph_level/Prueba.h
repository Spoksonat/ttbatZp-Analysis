...
class Prueba : public AnalyzerBase
{
 ...
 private:
  ...
  std::vector<const MCParticleFormat*> electron_final_state_array;
  std::vector<const MCParticleFormat*> muon_final_state_array;
  bool notMerged;
  ...
  MAbool is_electron_final_state(const MCParticleFormat* part){
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
     if ( (part->pdgid()!=-11)&&(part->pdgid()!=11) ) return false;
     return true; }

  MAbool is_muon_final_state(const MCParticleFormat* part){
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
     if ( (part->pdgid()!=-13)&&(part->pdgid()!=13) ) return false;
     return true; }

  MAbool cut_0(std::vector<const MCParticleFormat*> leptons, int pdgcode){
    
    bool cut = false;
    for (MAuint32 i=0;i<leptons.size();i++)
    {
      if( ( (leptons[i]->pdgid() == -pdgcode) || (leptons[i]->pdgid() == pdgcode) ) && (leptons[i]->momentum().Pt() > 35.0) &&  (TMath::Abs(leptons[i]->momentum().Eta()) < 2.4) )
      {
        cut = true;
        break;
      }
  ...
  MAbool cut_1(std::vector<const MCParticleFormat*> jets){
  ...
  MAbool cut_2(std::vector<const MCParticleFormat*> b){
  ...
  MAbool cut_3(const EventFormat& event){
    ...
    if ( PHYSICS->Transverse->EventMET(event.mc()) > 30 )
    {
      cut = true;
    }
